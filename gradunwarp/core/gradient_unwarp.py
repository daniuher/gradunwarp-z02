...
import os
import logging
from gradunwarp.core import (globals, utils)
...
def argument_parse_gradunwarp():
    ...
    p.add_argument('outpath', action='store',
                  help='The path to an output directory')
    p.add_argument('output_prefix', action='store',
...
class GradientUnwarpRunner(object):
    ...
    def write(self):
        self.unwarper.write(self.args.outpath, self.args.output_prefix)
